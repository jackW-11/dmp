...
object Point06_bycCient {
  def main(args: Array[String]): Unit = {
    ...
    val sql = "select " +
      "case when client=1 then 'android' else (case when client=2 then 'ios' else (case when client=3 then 'wp' else '其他' end)end)end clientname," +
    ...
    res.write.partitionBy("clientname")
      .mode(SaveMode.Overwrite)
      .json(outputPath)
    Sink2Mysql.tomysql(res,"byclient")
  }
}
