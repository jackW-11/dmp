...
object TagContext {
  def main(args: Array[String]): Unit = {
    ...
    val allUserId = df.rdd.map(row=>{
      // 获取所有ID
      val strList = Tag_User.getallUserId(row)
      (strList,row)
    })
    // 构建点集合
    val verties = allUserId.flatMap(row=>{
      // 获取所有数据
      val rows = row._2
      val adList = Tag01_AD.makeTags(rows)
      // 商圈
      val businessList = Tag07_Business.makeTags(rows)
      // 媒体标签
      val appList = Tag02_APP.makeTags(rows,broadValue)
      // 设备标签
      val devList = Tag04_Dev.makeTags(rows)
      // 地域标签
      val locList = Tag06_Location.makeTags(rows)
      // 关键字标签
      val kwList = Tag05_Keys.makeTags(rows,broadValues)
      // 获取所有的标签
