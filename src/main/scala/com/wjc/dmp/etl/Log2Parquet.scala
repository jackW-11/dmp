package com.wjc.dmp.etl

import com.wjc.dmp.util.{SchemaUtil, Sink2Mysql, String2Type}
import org.apache.spark.sql.{Row, SaveMode, SparkSession}

/**
  * 对数据进行读取，转换成parquet存储
  * */
object Log2Parquet {
  def main(args: Array[String]): Unit = {
    ...
    val df = spark.createDataFrame(rdd,SchemaUtil.structType)
    Sink2Mysql.tomysql(df,"df")//存到数据库
